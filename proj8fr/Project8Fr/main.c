...
//LCD_BACKLITE_DIMING = wheel_speed(R, 20);
    ...
    if(read_pointer != iot_rx_ring_wr){
        bufferDS[buffer_pointer] = IOT_Char_Rx[read_pointer];
                            buffer_pointer++;
                            if(++read_pointer >= (sizeof(IOT_Char_Rx))){
                                read_pointer = 0;
                            }
                            dscounter += 1;
        if((bufferDS[buffer_pointer-2] == '\r' && bufferDS[buffer_pointer-1] == '\n')){

            //Now have to handle the string coming in
            if(dscounter >= 12){ //if length is ten
                for(j = 0; j < 10; j++){
                    dsOUT[j] = bufferDS[j];
                }
                dsOUT[10] = 0x00;
            } else{
                dscounter= dscounter -2;
                for(j = 0; j < dscounter; j++){
                    dsOUT[j] = bufferDS[j];
                }
                for(j = dscounter; j < 10; j++){
                    dsOUT[j] = ' ';
                }
                dsOUT[10] = 0x00;
            }
                strcpy(display_line[1], dsOUT);
                display_changed = TRUE;
                buffer_pointer = 0;
                dscounter = 0;
                //initiliaze read pointer
            }
    }
...
//    strcpy(display_line[1], IOT_Char_Rx);
//    run_project8();
